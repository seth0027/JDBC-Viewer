 ...
 * @version Mar 1, 2020
 */
public class JDBCViewer extends Application {
	 ...
	 * {@link JDBCViewerV2#start(Stage)} method.
	 */
	...
	private Region createOptionsBar() throws SQLException {
		...
		ComboBox<String> tablesCombo = createComboBox(controller.getTableNames(), "Table", 0);

		controller.tableInUseProperty().bind(tablesCombo.getSelectionModel().selectedItemProperty());

		// this line of code binds the selected item in the combo box to the controllers
		// tableInUseProperty.
		...
		// call connect and then getTableNames on controller. we call getTableNames
		// again just to update the names. the list is already connected to ComboBox.
		// call setText on conectionStatus pass to it "connected".
		// don't forget connect throws an exception, so use try and catch.
		// if exception is thrown, setText on conectionStatus pass to it "failed: " +
		// ex.getMessage() and call ex.printStackTrace();
		...
		// if controller is not connected call setText on conectionStatus pass to it
		// "must connect first" and get out.
		// call setText on conectionStatus pass to it "searching".
		// call search on controller and pass searchText.getText().trim(). store the
		// result in a variable called list.
		// if list is not null setText on conectionStatus to "populating table" and call
	...
	private void populateTable(List<List<Object>> list) throws SQLException {
		...
		// method data.getValue() return the list we added in second line of this
		// method.
